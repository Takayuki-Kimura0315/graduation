...
def Nscheduling(ab):#通常のナーススケジューリング
    ...
    Nightsum={}
    for n in range(N):
        Nightsum[n]=model.addVar(vtype="I",lb=0)
    ...
    #夜勤回数の保存
    for n in range(N):
        model.addConstr(Nightsum[n]==quicksum(allocation[n,i,j,Night] for i in range(4) for j in range(7)))
    #ついでに最大夜勤回数を上から押さえつける
    for n in range(N):
        model.addConstr(Nightsum[n]<=t)
    ...
    model.setObjective(t/10+quicksum(shortage[i,j,k]*Shortagecost[i,j,k]+surplus[i,j,k]*Surpluscost[i,j,k] for i in range(4) for j in range(7) for k in range(4)),GRB.MINIMIZE)
    model.optimize()
