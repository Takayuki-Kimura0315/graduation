...
def Nscheduling(ab):#通常のナーススケジューリング
    ...
    #労働時間
    for n in range(N):
        model.addConstr(quicksum(Ho[k]*allocation[n,i,j,k] for i in range(4) for j in range(7) for k in range(4))==Hosum[n])
        model.addConstr(Hosum[n]<=Homax[ESNurse[n]])
        #model.addConstr(Hosummax==max((Hosum[n])for n in range(N)))
    ...
    for n in range(N):
        if ESNurse[n]==0:
            model.addConstr(Hosum[n]<=s)
        elif ESNurse[n]==1:
            model.addConstr(Hosum[n]<=t)
        elif ESNurse[n]==2:
            model.addConstr(Hosum[n]<=u)

    #Hosum={}
    #for n in range(N):
        #model.addConstr(Hosum[n]==quicksum(Ho[k]*allocation[n,i,j,k] for i in range(4) for j in range(7) for k in range(4)))
    #temp = (max(Hosum[n]) for n in range(N))
    #print((quicksum(shortage[i,j,k]*Shortagecost[i,j,k]+surplus[i,j,k]*Surpluscost[i,j,k] for i in range(4) for j in range(7) for k in range(4))))
    model.setObjective(((s+t+u)/10000)+(quicksum(shortage[i,j,k]*Shortagecost[i,j,k]+surplus[i,j,k]*Surpluscost[i,j,k] for i in range(4) for j in range(7) for k in range(4))),
        GRB.MINIMIZE)
    model.optimize()
    ...
    cost=model.ObjVal-((s.X+t.X+u.X)/10000)
    print("コストのみ")
    print(cost)
